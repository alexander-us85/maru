...
static subr(double_string)
{
    ...
    wchar_t buf[32];
    swnprintf(buf, 32, L"%f", (wchar_t)getDouble(arg));
    return newString(buf);
}
...
static subr(subr)
{
    ...
    HANDLE currentModuleHandle = GetModuleHandle(NULL);
    FARPROC addr = GetProcAddress(currentModuleHandle, sym);
    ...
    return newSubr(name, (imp_t)addr, sig);
}
...
static subr_ent_t subr_tab[256];
...
static void loader(FILE *in, void *ptr)
{
	...
	case Symbol: {
	    ...
	    set(obj, Symbol,bits, _wcsdup(str));
	    break;
	}
	case Subr: {
